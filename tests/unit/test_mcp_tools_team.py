...
class TestTeamRegisterAgent:
    ...
    @pytest.mark.asyncio
    async def test_register_agent_success_returns_ok(self, session: StatefulSession) -> None:
        result = await team_register_agent(session, id="a1", name="Alice", role="coder")
        assert result["ok"] is True
        assert result["data"]["agent_id"] == "a1"

    @pytest.mark.asyncio
    async def test_register_agent_duplicate_returns_error(self, session: StatefulSession) -> None:
        await team_register_agent(session, id="a1", name="Alice", role="coder")
        result = await team_register_agent(session, id="a1", name="Alice2", role="coder")
...
class TestTeamListAgents:
    ...
    @pytest.mark.asyncio
    async def test_list_agents_empty_returns_empty(self, session: StatefulSession) -> None:
        result = await team_list_agents(session)
        assert result["ok"] is True
        assert result["data"] == []

    @pytest.mark.asyncio
    async def test_list_agents_returns_registered(self, session: StatefulSession) -> None:
        await team_register_agent(session, id="a1", name="Alice", role="coder")
...
class TestTeamClaimTask:
    ...
    @pytest.mark.asyncio
    async def test_claim_task_no_tasks_returns_error(self, session: StatefulSession) -> None:
        result = await team_claim_task(session)
        assert result["ok"] is False
        assert "No tasks available" in result["error"]

    @pytest.mark.asyncio
    async def test_claim_task_gets_highest_priority(self, session: StatefulSession) -> None:
    ...
    @pytest.mark.asyncio
    async def test_claim_task_transitions_to_in_progress(self, session: StatefulSession) -> None:
...
class TestTeamListTasks:
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_filter_by_priority(self, session: StatefulSession) -> None:
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_filter_by_assignee(self, session: StatefulSession) -> None:
        await team_create_task(session, id="t1", title="Unassigned")
        await team_create_task(session, id="t2", title="Assigned", assignee_agent_id="a1")
