...
class TestPartitionCreation:
    ...
    def test_create_partition_returns_budget_tracker(self) -> None:
        tracker = PartitionedBudgetTracker(global_policy=BudgetPolicy(max_total_usd=100.0))
...
class TestPerGoalRecording:
    ...
    def test_record_accumulates_in_partition(self) -> None:
        tracker = PartitionedBudgetTracker(global_policy=BudgetPolicy(max_total_usd=100.0))
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=50.0))
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=10.0))
...
class TestGlobalRollup:
    ...
    def test_total_cost_sums_across_partitions(self) -> None:
        ...
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=50.0))
        tracker.create_partition("goal-b", BudgetPolicy(max_total_usd=50.0))
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=30.0))
        tracker.record("goal-b", CostRecord(agent_id="dev2", task_id="t2", cost_usd=40.0))
        assert tracker.total_cost() == pytest.approx(70.0)

    def test_is_exceeded_true_when_global_limit_hit(self) -> None:
        tracker = PartitionedBudgetTracker(global_policy=BudgetPolicy(max_total_usd=50.0))
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=100.0))
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=50.0))
    ...
    def test_is_exceeded_false_when_no_limit(self) -> None:
        ...
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=999.0))
        assert tracker.is_exceeded() is False

    def test_is_partition_exceeded_true_when_partition_limit_hit(self) -> None:
        tracker = PartitionedBudgetTracker(global_policy=BudgetPolicy(max_total_usd=200.0))
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=20.0))
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=25.0))
    ...
    def test_partition_exceeded_does_not_affect_other_partition(self) -> None:
        ...
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=15.0))
        assert tracker.is_partition_exceeded("goal-a") is True
...
class TestNamespacedEvents:
    ...
    async def test_warning_event_has_namespace_prefix(self) -> None:
        ...
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=10.0, warn_at_percent=50.0))
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=6.0))
    ...
    async def test_exceeded_event_has_namespace_prefix(self) -> None:
        ...
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=10.0, warn_at_percent=80.0))
        # First record: triggers warning at 90%
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=9.0))
        # Second record: exceeds budget
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t2", cost_usd=5.0))
        await asyncio.sleep(0.01)
    ...
    async def test_other_partitions_continue_after_one_exceeds(self) -> None:
        ...
        tracker.record("goal-b", CostRecord(agent_id="dev2", task_id="t2", cost_usd=5.0))
        assert tracker.is_partition_exceeded("goal-b") is False
    ...
    async def test_global_warning_event_fires(self) -> None:
        ...
        tracker.create_partition("goal-a", BudgetPolicy(max_total_usd=100.0))
        tracker.record("goal-a", CostRecord(agent_id="dev1", task_id="t1", cost_usd=60.0))
        await asyncio.sleep(0.01)
...
class TestBackwardCompat:
    ...
    def test_without_partitions_total_cost_works(self) -> None:
        ...
        tracker.record("orphan", CostRecord(agent_id="dev1", task_id="t1", cost_usd=5.0))
        assert tracker.total_cost() == pytest.approx(5.0)

    def test_without_partitions_is_exceeded_works(self) -> None:
        tracker = PartitionedBudgetTracker(global_policy=BudgetPolicy(max_total_usd=10.0))
        tracker.record("orphan", CostRecord(agent_id="dev1", task_id="t1", cost_usd=15.0))
        assert tracker.is_exceeded() is True
