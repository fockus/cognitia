...
async def test_exec_valid_arithmetic():
    ...
    res = await exec_code(code="print(sum(range(100)))")
...
async def test_exec_stdlib_import():
    ...
    res = await exec_code(code="import os; print(os.path.exists('/'))")
    assert res["ok"] is True
...
async def test_exec_timeout_kills_long_running():
    ...
    res = await exec_code(code="import time; time.sleep(100)", timeout_seconds=2)
...
async def test_exec_nonzero_exit_code():
    ...
    res = await exec_code(code="import sys; sys.exit(42)")
    assert res["ok"] is False
