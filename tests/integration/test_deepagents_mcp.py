"""Integration: DeepAgentsRuntime MCP tool injection.

DeepAgentsRuntime(mcp_servers={"srv": ...}) + McpBridge.
Mock McpClient.list_tools (HTTP граница -- OK to mock).
Проверить: MCP tools появляются в selected_tools, executor создан.
"""
...
class TestDeepAgentsMcpToolInjection:
    ...
    def test_deepagents_mcp_bridge_created_when_servers_provided(self) -> None:
        """McpBridge создается при передаче mcp_servers."""
    ...
    def test_deepagents_no_mcp_bridge_without_servers(self) -> None:
        """Без mcp_servers McpBridge не создается."""
        runtime = DeepAgentsRuntime(
            config=RuntimeConfig(runtime_name="deepagents"),
        )
    ...
    async def test_deepagents_mcp_tool_injection(self) -> None:
        """MCP tools появляются через McpBridge.discover_all_tools."""
        mcp_tools = [
            ToolSpec(
        ...
        # Mock discover_all_tools на bridge (HTTP граница)
        runtime._mcp_bridge.discover_all_tools = AsyncMock(return_value=mcp_tools)  # type: ignore[union-attr]

        # select_active_tools в hybrid mode сохраняет все tools
        user_tools = [
            ToolSpec(
                name="local_calc",
        ...
        # В hybrid mode все tools сохраняются
        assert len(selected) == 1  # select_active_tools не добавляет MCP — это делает run()

        # Проверяем что bridge.discover_all_tools возвращает MCP tools
    ...
    async def test_deepagents_mcp_executor_created_for_discovered_tools(self) -> None:
        """create_tool_executor создает callable для каждого MCP tool."""
        runtime = DeepAgentsRuntime(
            config=RuntimeConfig(runtime_name="deepagents"),
        ...
        # create_tool_executor возвращает callable
    ...
    def test_deepagents_select_tools_portable_filters_builtins(self) -> None:
        """portable mode фильтрует native built-in tools."""
        runtime = DeepAgentsRuntime(
        ...
        # portable mode фильтрует native built-ins (read_file)
        selected_names = [t.name for t in selected]
        assert "custom_tool" in selected_names

    def test_deepagents_select_tools_hybrid_keeps_all(self) -> None:
        """hybrid mode сохраняет все tools включая native builtins."""
        runtime = DeepAgentsRuntime(
            config=RuntimeConfig(
                runtime_name="deepagents",
                feature_mode="hybrid",
