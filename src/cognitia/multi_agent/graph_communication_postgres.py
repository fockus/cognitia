...
class PostgresGraphCommunication:
    ...
    async def _store_msg(self, session: AsyncSession, msg: GraphMessage) -> None:
        ...
        await session.execute(
            ...
            {
                "id": msg.id, "from_id": msg.from_agent_id,
                "to_id": msg.to_agent_id, "channel": msg.channel.value,
    ...
    def _row_to_msg(self, r: Any) -> GraphMessage:
        ...
        return GraphMessage(
            ...
            created_at=float(r[6]), metadata=meta,
        )

    async def get_inbox(self, agent_id: str) -> list[GraphMessage]:
        async with self._session() as session:
            rows = (await session.execute(
                text("SELECT id, from_agent_id, to_agent_id, channel, content, task_id, created_at, metadata "
