...
from cognitia.runtime.openai_agents.tool_bridge import ToolExecutorFn, toolspecs_to_agent_tools
...
class OpenAIAgentsRuntime:
        ...
        *,
        ...
        try:
            ...
            yield RuntimeEvent.error(
                RuntimeErrorData(
                    kind="runtime_crash",
                    message="openai-agents not installed. Run: pip install cognitia[openai-agents]",
                    recoverable=False,
                )
            )
            return
        ...
        # Convert Cognitia tools to OpenAI Agent tools
    ...
    def _build_input(messages: list[Message]) -> str | list[dict[str, str]]:
        """Build input for Runner from Cognitia messages.
