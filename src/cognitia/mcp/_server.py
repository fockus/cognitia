"""Cognitia MCP Server -- FastMCP-based STDIO server.
...
def create_server(*, mode: str = "auto") -> Any:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install cognitia[mcp]"
        ) from exc
    ...
    mcp = FastMCP(
        "cognitia",
        description=(
            "Cognitia AI Agent Framework -- memory, plans, team coordination, "
    ...
    @mcp.tool()
    async def cognitia_memory_upsert_fact(
    ...
    @mcp.tool()
    async def cognitia_memory_get_facts(
    ...
    @mcp.tool()
    async def cognitia_memory_save_message(
    ...
    @mcp.tool()
    async def cognitia_memory_get_messages(
    ...
    @mcp.tool()
    async def cognitia_memory_save_summary(
    ...
    @mcp.tool()
    async def cognitia_memory_get_summary(
    ...
    @mcp.tool()
    async def cognitia_plan_create(
    ...
    @mcp.tool()
    async def cognitia_plan_get(plan_id: str) -> dict[str, Any]:
        """Load a plan by its ID."""
        return await plan_get(session, plan_id)

    @mcp.tool()
    async def cognitia_plan_list(
    ...
    @mcp.tool()
    async def cognitia_plan_approve(
    ...
    @mcp.tool()
    async def cognitia_plan_update_step(
    ...
    @mcp.tool()
    async def cognitia_team_register_agent(
        id: str,
        name: str,
        role: str,
    ...
    @mcp.tool()
    async def cognitia_team_list_agents(
    ...
    @mcp.tool()
    async def cognitia_team_create_task(
        id: str,
        title: str,
        description: str = "",
        priority: str = "MEDIUM",
    ...
    @mcp.tool()
    async def cognitia_team_claim_task(
        assignee_agent_id: str | None = None,
    ) -> dict[str, Any]:
    ...
    @mcp.tool()
    async def cognitia_team_list_tasks(
    ...
    if resolved == "full":
        ...
        @mcp.tool()
        async def cognitia_agent_create(
            system_prompt: str,
            model: str = "sonnet",
            runtime: str = "thin",
        ...
        @mcp.tool()
        async def cognitia_agent_query(agent_id: str, prompt: str) -> dict[str, Any]:
            """Send a prompt to an existing agent. Requires API key."""
            return await agent_query(session, agent_id, prompt)

        @mcp.tool()
        async def cognitia_agent_list() -> dict[str, Any]:
    ...
    @mcp.tool()
    async def cognitia_status() -> dict[str, Any]:
        """Get Cognitia MCP server status."""
...
def main() -> None:
    """Entry point for cognitia-mcp and python -m cognitia.mcp."""
