...
_ATTR_COGNITIA_RUNTIME = "cognitia.runtime"
_ATTR_COGNITIA_SESSION_ID = "cognitia.session_id"
...
def _try_import_otel() -> tuple[Any, Any]:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install cognitia[otel]"
        ) from None
...
class OTelExporter:
    ...
    def _base_attributes(self) -> dict[str, Any]:
        ...
        attrs: dict[str, Any] = {_ATTR_GEN_AI_SYSTEM: "cognitia"}
        if self._runtime_name:
            attrs[_ATTR_COGNITIA_RUNTIME] = self._runtime_name
        if self._session_id:
            attrs[_ATTR_COGNITIA_SESSION_ID] = self._session_id
        return attrs
    ...
    def _on_tool_start(self, data: dict[str, Any]) -> None:
        ...
        span = self._tracer.start_span(
            name=f"cognitia.tool.{tool_name}",
            attributes=attrs,
        )
